...
function getHTMLMediaElement(mediaElement, config) {
    ...
    if (buttons.has('mute-video')) {
       // var muteVideo = document.createElement('div');
         var muteVideo = document.getElementById('cus-mutvid');
        muteVideo.className = ' ' + (config.toggle.has('mute-video') ? 'unmute-video selected' : 'mute-video');
   ...
   if (buttons.has('stop')) {
    ...
    if (buttons.has('full-screen')) {
        ...
        zoom.className = 'controlfs ' + (config.toggle.has('zoom-in') ? 'zoom-out selected' : 'zoom-in');
    ...
    var getvwid = $('#videos-container').width();
    // mediaElementContainer.style.width = config.width + 'px';
    mediaElementContainer.style.width = getvwid+'px';
   // mediaElementContainer.style.height = getvwid+'px';

    if (config.height) {
        mediaBox.style.height = config.height + 'px';
    }

    mediaBox.querySelector('video').style.maxHeight = getvwid+'px';
